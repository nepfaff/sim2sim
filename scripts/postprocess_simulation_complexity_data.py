"""
Script for post-processing the dataset generated by
'collect_simulation_complexity_data.py.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The top level path for the data generated by "
    ...
    for perturbation_entry in os.scandir(args.data_path):
        ...
        for run_entry in os.scandir(perturbation_entry.path):
            ...
            outer_states = np.load(
                os.path.join(time_logs_dir, "outer_manipuland_poses.npy")
            )
            inner_states = np.load(
                os.path.join(time_logs_dir, "inner_manipuland_poses.npy")
            )
            final_displacement_errors = [
                final_displacement_error(outer, inner)
                for outer, inner in zip(outer_states, inner_states)
            ]  # Shape (M, 1) where M is the number of manipulands
            # Take mean over manipulands
            final_pose_error_magnitudes.append(np.mean(final_displacement_errors))
