...
def process_perturbation(perturbation_entry_path: str) -> None:
    ...
    for run_entry in os.scandir(perturbation_entry_path):
        if run_entry.is_dir():
            times_path = os.path.join(run_entry.path, "time_logs", "outer_manipuland_pose_times.txt")
            if not os.path.exists(times_path):
                print(f"Data incomplete for {run_entry}")
                continue
            times = np.loadtxt(times_path)
            outer_manipuland_states_path = os.path.join(run_entry.path, "time_logs", "outer_manipuland_poses.txt")
            inner_manipuland_states_path = os.path.join(run_entry.path, "time_logs", "inner_manipuland_poses.txt")
