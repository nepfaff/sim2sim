...
class GMMMeshProcessor(MeshProcessorBase):
    """
    Replaces the mesh with spheres obtained from fitting GMMs using Expectation
    Maximization.
    """
    ...
    def __init__(
        ...
        :param gmm_em_params: Parameters for GMM EM fitting. All must be valid arguments
            for sklearn.mixture.GaussianMixture.
        :param threshold_std: The standard deviation to use as a threshold for
            converting a GMM into a mesh.
        """
    ...
    def process_meshes(
        ...
        for mesh in meshes:
            ...
            for idx in range(self._gmm_em_params["n_components"]):
                ...
                ellipsoid = Ellipsoid(
                    center=c_i,
                    radius=r_i * self._threshold_std,
                    scale=1,
                    transform=rot_mat,
                )
