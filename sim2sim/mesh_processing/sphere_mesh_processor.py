...
class SphereMeshProcessor(MeshProcessorBase):
    ...
    def __init__(self, logger: DynamicLogger, target_sphere_num: int, visualize: bool):
        """
        :param target_sphere_num: The number of spheres that the simplified mesh should
            contain.
    ...
    def process_meshes(
        ...
        for mesh in meshes:
            ...
            # subsampled_pts = pointnet2_utils.furthest_point_sample(
            # points, self._target_sphere_num)
            ...
            for idx in range(self._target_sphere_num):
                ...
                within_sphere_points = (
                    remaining_points[0][output].detach().cpu().numpy()
                )
